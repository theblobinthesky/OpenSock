...
DecodingResult NpyDataDecoder::loadFromMemory(
    ...
    // cnpy memory header uses 2-byte header length at offset 8; data follows header at offset 10+len
    const uint16_t header_len = *reinterpret_cast<const uint16_t *>(inputData + 8);
    const size_t data_offset = 10 + static_cast<size_t>(header_len);
